function checkForInput(inputText) {
    ...
    if (inputText != "") {
        // check if contains non-English characters (punctuation excluded)
        const pattern = new RegExp(/[\x00-\x7F]+/);
        const allWrittenInEnglish = pattern.test(inputText);

        if (allWrittenInEnglish) {
            // check if user input a url, if true, then return invalid -----
            // since the app only takes text input (due to the limitation of the API)
            ...
            console.log("here is the url test");
            
            // if the input is a url, return false, else return true
            return !urlPattern.test(inputText);

        } else {

            console.log("contains non-english characters")
            return false;
